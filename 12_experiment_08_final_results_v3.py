...
def discover_baseline_results() -> pd.DataFrame:
    """
    Discover conventional baseline result tables from Experiment 2.

    Earlier Experiment 2 scripts used different filenames and column names,
    so discovery must be based on table content rather than filename alone.

    A CSV is accepted when:
    - it contains a model/classifier/algorithm identifier column; and
    - it contains at least one recognizable predictive-performance metric.

    Every CSV under Experiment_02_Baseline_Models is inspected. Non-result
    tables such as split assignments, feature lists, or prediction files are
    rejected automatically by the schema test.
    """

    if not BASELINE_ROOT.exists():
        LOGGER.warning(
            "Baseline root does not exist: %s",
            BASELINE_ROOT,
        )
        return pd.DataFrame()
    ...
    for path in sorted(
        ...
        if not identifier_columns or not metric_columns:
            continue

        # Reject obvious per-record prediction tables even if their names
        # happen to contain "model". Baseline summary tables should have
        # relatively few rows compared with prediction files.
        if len(frame) > 10_000:
            continue

        copy = frame.copy()
        copy["_source_file"] = str(path)
        copy["_model_identifier_columns"] = ",".join(
            str(column)
            for column in identifier_columns
        )
        copy["_metric_columns_detected"] = ",".join(
            str(column)
        ...
        LOGGER.info(
            "Accepted baseline table: %s | rows=%d | model columns=%s | metric columns=%s",
            path,
            len(frame),
            identifier_columns,
            metric_columns,
    ...
    LOGGER.info(
        "Baseline discovery inspected %d CSV files under %s",
        inspected,
        BASELINE_ROOT,
    )
    ...
    if not candidates:
        LOGGER.warning(
            "No conventional baseline result table was discovered under %s",
            BASELINE_ROOT,
        )
        return pd.DataFrame()
    ...
    LOGGER.info(
        "Baseline consolidation loaded %d row(s) from %d accepted table(s).",
        len(result),
        len(candidates),
    )
